...
12. **Always restart the application after every version change**: After every version bump — major, minor, or patch — the running container must be brought up to date by rebuilding and recreating it. Run `docker compose up --build -d` (the `--build` ensures the image is rebuilt; `-d` keeps it detached). This must happen **after** the commit/push from rule 2, on every release, with no exceptions. Verify by hitting `GET /health` and confirming the returned `version` field matches the new `VERSION` file.
...
14. **Multiple-choice next steps**: After completing each user prompt, offer a multiple-choice menu (typically 3–4 options) for what to do next via the `AskUserQuestion` tool. Options should be specific, actionable, and informed by the current state of the work — open TODOs in `docs/TODO.md`, recent changes, balance dials, follow-up cleanup, etc. — so the user can pick a direction to continue without composing one from scratch.
